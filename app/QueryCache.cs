...
namespace Orbis
{
    internal static class QueryCache
    {
        ...
        public static bool TrySearch(string query, out List<GameHit> hits, string sourceStamp = "")
        {
            hits = null;
            string body;
            if (!TryRead("s-" + Key("group-v1|" + sourceStamp + "|" + (query ?? "").Trim().ToLowerInvariant()), out body)) return false;
        ...
        public static void PutSearch(string query, List<GameHit> hits, string sourceStamp = "")
        {
            ...
            Write("s-" + Key("group-v1|" + sourceStamp + "|" + (query ?? "").Trim().ToLowerInvariant()), sb.ToString());
        }
            ...
            using (var reader = new StringReader(body))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    var cand = new PackageCandidate
                    {
                        ...
                        ArchivePasswords = p.Length > 23 ? p[23] : "",
        ...
        public static void PutResolve(string titleId, List<PackageCandidate> candidates, string sourceStamp = "", string region = "", string sourceId = "")
        {
            ...
            for (int i = 0; i < candidates.Count; i++)
            {
                ...
                sb.Append(Esc(c.SourceId)).Append('\t').Append(Esc(c.CandidateId)).Append('\t')
                    ...
                    .Append(Esc(c.ResolutionError)).Append('\t').Append(Esc(c.ArchivePasswords)).Append('\n');
            }
