...
namespace Orbis
{
    internal static class ResolverPageCache
    {
            ...
            Func<Uri, bool> allowOrigin = null)
        {
            ...
            if (!string.IsNullOrEmpty(bearer)) return NetHttp.GetStringDirect(url, timeout, referer, bearer, userAgent, allowOrigin: allowOrigin);
            ...
            string body;
            try { body = SourceHttps.GetString(url, timeout, referer, userAgent, allowOrigin: allowOrigin); }
