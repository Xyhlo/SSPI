...
namespace Orbis
{
    internal static class SourceHttps
    {
        ...
        public static string GetString(string url,int timeoutMs,string referer,string userAgent)
        {
            ...
            if(uri.Scheme=="http")return PlainFetch!=null?PlainFetch(url,timeoutMs,referer,null,userAgent):NetHttp.GetStringDirect(url,timeoutMs,referer,null,userAgent);
            if(ModuleFetch!=null)return ModuleFetch(url,timeoutMs,referer,userAgent);
            return NativeHttp.GetString(url,timeoutMs,referer,null,MaxBytes,userAgent);
        }
    }
}
