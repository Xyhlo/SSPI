...
void gs_http_close(GsHttp *h)
{
    ...
    // Preserve the live lock and cancellation state while clearing the
    // connection cache. A full reset is only safe before sharing this object.
...
static int http_open_request(GsHttp *h,const char *url,const char *bearer,int64_t start,int64_t end)
{
    char current[8192],initial[512],origin[512],value[8192];int rc=-1;
    if(strlen(url)>=sizeof(current)||gs_http_origin(url,initial,sizeof(initial)))return -2;
    ...
    for(int hop=0;hop<=8;hop++) {
        ...
        h->start=h->end=h->total=h->length=-1;h->reused=0;h->stage="origin";
        ...
        if(strcmp(origin,h->origin)) {
            ...
            if(http_recv_block) {
                h->recv_block_rc=http_recv_block(h->template_id,256*1024);
                if(!h->recv_block_rc)h->recv_block=256*1024;
                else {
                    h->recv_block_rc=http_recv_block(h->template_id,64*1024);
                    if(!h->recv_block_rc)h->recv_block=64*1024;
                }
            }
            h->stage="timeouts";
        ...
        if(start>=0) {snprintf(value,sizeof(value),"bytes=%lld-%lld",(long long)start,(long long)end);if((rc=http_header(request,"Range",value,0))<0)return rc;}
        ...
        // expires, visit the original URL once; never forward its bearer cross-origin.
        if(cached && (h->status==401||h->status==403||h->status==404||h->status==410)) {
...
int gs_http_open(GsHttp *h,const char *url,const char *bearer,int64_t start,int64_t end)
{
    ...
    int rc=http_open_request(h,url,bearer,start,end);
