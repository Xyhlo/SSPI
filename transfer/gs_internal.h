...
typedef struct {
    ...
    int storage_errno;
    char url[8192], bearer[2048], dest[1024], part[1100], map[1100], lock_path[1100], origin[512];
...
int gs_http_open(GsHttp *http, const char *url, const char *bearer, int64_t start, int64_t end);
...
int gs_store_open(GsJobState *job, const unsigned char identity[32]);
